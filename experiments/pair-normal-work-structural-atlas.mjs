...
const proto=K.Kernel.prototype,retainedRun=proto.run,retainedEqual=proto.equal,retainedWhnf=proto.whnf,retainedInfer=proto.infer,retainedNormal=proto.normal;
...
const normalWork={tailVisits:0,identityUnique:0,identitySeen:new WeakSet(),structural:new Map(),bytes:new Map(),ctxKeys:new Map()};
function normalCtxKey(kernel,ctx) {
  kernel._normalCtxIds ??= new WeakMap(); kernel._nextNormalCtxId ??= 1;
  const parts=[];
  for(const entry of ctx) {
    if(entry!==null && (typeof entry==="object" || typeof entry==="function")) {
      let id=kernel._normalCtxIds.get(entry);
      if(id===undefined){ id=kernel._nextNormalCtxId++; kernel._normalCtxIds.set(entry,id); }
      parts.push(id);
    } else parts.push(typeof entry+":"+String(entry));
  }
  return parts.join(",");
}
function normalCacheSlot(kernel,e,ctxKey) {
  if(!Array.isArray(e)) return null;
  kernel._normalCache ??= new WeakMap();
  let byCtx=kernel._normalCache.get(e);
  if(!(byCtx instanceof Map)){byCtx=new Map();kernel._normalCache.set(e,byCtx);}
  return {has:()=>byCtx.has(ctxKey),get:()=>byCtx.get(ctxKey),set:v=>byCtx.set(ctxKey,v)};
}
function recordNormalVisit(k,e,ckey){
  if(k.steps<2000000 || !Array.isArray(e)) return;
  normalWork.tailVisits++;
  if(!normalWork.identitySeen.has(e)){normalWork.identitySeen.add(e);normalWork.identityUnique++;}
  const js=JSON.stringify(e);
  normalWork.structural.set(js,(normalWork.structural.get(js)??0)+1);
  if(!normalWork.bytes.has(js))normalWork.bytes.set(js,js.length);
  normalWork.ctxKeys.set(ckey,(normalWork.ctxKeys.get(ckey)??0)+1);
}
function instrumentedLocalNormal(root){
  this._normalCache ??= new WeakMap();
  this._normalCtxIds ??= new WeakMap();
  this._nextNormalCtxId ??= 1;
  if(!Array.isArray(root)) return root;
  const ckey=normalCtxKey(this,this._activeCtx??[]);
  ...
  while(work.length){
    ...
    const requested=f.e;
    recordNormalVisit(this,requested,ckey);
    const slot=normalCacheSlot(this,requested,ckey);
    if(slot?.has()){vals.push(slot.get());continue;}
    this.tick();
    const e=this.whnf(requested);
    if(e!==requested){
      const reducedSlot=normalCacheSlot(this,e,ckey);
      if(reducedSlot?.has()){
        const out=reducedSlot.get();slot?.set(out);vals.push(out);continue;
      }
    }
    if(["sort","var","const","nat","strlit"].includes(e[0])){
      slot?.set(e);if(e!==requested)normalCacheSlot(this,e,ckey)?.set(e);vals.push(e);continue;
    }
    if(e[0]==="proj"){
      work.push({kind:"build",tag:"proj",name:e[1],index:e[2],slot});
      work.push({kind:"visit",e:e[3]});continue;
    }
...
function normalWorkSummary(){
  const top=[...normalWork.structural.entries()].sort((a,b)=>b[1]-a[1]).slice(0,40)
    .map(([term,count])=>({count,bytes:normalWork.bytes.get(term),term:term.slice(0,1200)}));
  return {tailVisits:normalWork.tailVisits,identityUnique:normalWork.identityUnique,
    structuralUnique:normalWork.structural.size,top,
    ctxKeys:[...normalWork.ctxKeys.entries()].sort((a,b)=>b[1]-a[1]).slice(0,20)};
}
function install(enabled){
  proto.run=retainedRun;proto.equal=retainedEqual;proto.whnf=retainedWhnf;proto.infer=retainedInfer;proto.normal=retainedNormal;
  if(!enabled)return;
  proto.run=function(...args){
    ...
    normalWork.tailVisits=0;normalWork.identityUnique=0;normalWork.identitySeen=new WeakSet();normalWork.structural=new Map();normalWork.bytes=new Map();normalWork.ctxKeys=new Map();
  ...
  proto.normal=function(e){
    if(this.localDefs===true)return instrumentedLocalNormal.call(this,e);
...
function evaluate(budget,enabled,rows){
  ...
  return {budget,enabled,wrong,totalSteps,totalConstructed,elapsed_ms:Date.now()-t0,stats:d,results,normalWork:normalWorkSummary()};
}
const thresholds=[];
for(const budget of [2100000]){
  thresholds.push(evaluate(budget,true,focusRows));
}
install(false);
console.log("PAIR_NORMAL_WORK_STRUCTURAL_ATLAS_THRESHOLDS "+JSON.stringify(thresholds));

console.log("PAIR_NORMAL_WORK_STRUCTURAL_ATLAS "+JSON.stringify(thresholds));
process.exit(0);
