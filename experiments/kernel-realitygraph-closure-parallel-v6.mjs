...
const closureMode=process.env.KERNEL_RG_CLOSURE_MODE??"trace";
if(!["trace","assume"].includes(closureMode))throw new Error("bad closure mode");
...
let closureScout={attempted:0,completed:0,failed:0,last:null,stuckRecs:[],assumedParamMatches:0};
...
function derefClosure(c){
  let x=c,guard=0;
  while(Array.isArray(x.term)&&x.term[0]==="var"&&x.term[1]<x.env.length){
    if(++guard>10000)throw new Error("closure-deref-cycle");
    x=x.env[x.term[1]];
  }
  return x;
}
function closureSame(a,b){
  let neutralNext=1,steps=0;
  const sameC=(x,y)=>{
    if(++steps>200000)throw new Error("closure-same-budget");
    x=derefClosure(x);y=derefClosure(y);
    if(x.term===y.term&&x.env===y.env)return true;
    const p=x.term,q=y.term;
    if(!Array.isArray(p)||!Array.isArray(q)||p[0]!==q[0]||p.length!==q.length)return false;
    switch(p[0]){
      case "neutral": return p[1]===q[1];
      case "sort": return pureSame(p[1],q[1]);
      case "const": return p[1]===q[1]&&pureSame(p[2]??[],q[2]??[]);
      case "nat": case "strlit": return p[1]===q[1];
      case "var": return p[1]===q[1]&&x.env.length===y.env.length;
      case "app":
        return sameC(C(p[1],x.env),C(q[1],y.env))&&sameC(C(p[2],x.env),C(q[2],y.env));
      case "proj":
        return p[1]===q[1]&&p[2]===q[2]&&sameC(C(p[3],x.env),C(q[3],y.env));
      case "pi": case "lam": {
        if(!sameC(C(p[1],x.env),C(q[1],y.env)))return false;
        const n=C(["neutral",neutralNext++],[]);
        return sameC(C(p[2],[n,...x.env]),C(q[2],[n,...y.env]));
      }
      case "let": {
        if(!sameC(C(p[1],x.env),C(q[1],y.env)))return false;
        if(!sameC(C(p[2],x.env),C(q[2],y.env)))return false;
        return sameC(C(p[3],[C(p[2],x.env),...x.env]),C(q[3],[C(q[2],y.env),...y.env]));
      }
...
function closureDiff(a,b){
  let neutralNext=1,steps=0;
  const go=(x,y,path)=>{
    if(++steps>50000)return {path,reason:"budget"};
    x=derefClosure(x);y=derefClosure(y);
    if(x.term===y.term&&x.env===y.env)return null;
    const p=x.term,q=y.term;
    if(!Array.isArray(p)||!Array.isArray(q))return {path,reason:"nonarray",p:String(p),q:String(q)};
    if(p[0]!==q[0]||p.length!==q.length)return {path,reason:"tag",p:p[0],q:q[0],pEnv:x.env.length,qEnv:y.env.length};
    switch(p[0]){
      case "neutral": return p[1]===q[1]?null:{path,reason:"neutral",p:p[1],q:q[1]};
      case "sort": return pureSame(p[1],q[1])?null:{path,reason:"sort"};
      case "const": return p[1]===q[1]&&pureSame(p[2]??[],q[2]??[])?null:{path,reason:"const",p:p[1],q:q[1]};
      case "nat": case "strlit": return p[1]===q[1]?null:{path,reason:p[0],p:p[1],q:q[1]};
      case "var": return p[1]===q[1]&&x.env.length===y.env.length?null:{path,reason:"free-var",p:p[1],q:q[1],pEnv:x.env.length,qEnv:y.env.length};
      case "app": return go(C(p[1],x.env),C(q[1],y.env),path+".fn")??go(C(p[2],x.env),C(q[2],y.env),path+".arg");
      case "proj":
        if(p[1]!==q[1]||p[2]!==q[2])return {path,reason:"proj-meta"};
        return go(C(p[3],x.env),C(q[3],y.env),path+".obj");
      case "pi": case "lam": {
        const d=go(C(p[1],x.env),C(q[1],y.env),path+".dom");if(d)return d;
        const n=C(["neutral",neutralNext++],[]);
        return go(C(p[2],[n,...x.env]),C(q[2],[n,...y.env]),path+".body");
      }
      case "let": {
        const d1=go(C(p[1],x.env),C(q[1],y.env),path+".type");if(d1)return d1;
        const d2=go(C(p[2],x.env),C(q[2],y.env),path+".value");if(d2)return d2;
        return go(C(p[3],[C(p[2],x.env),...x.env]),C(q[3],[C(q[2],y.env),...y.env]),path+".body");
      }
      default:return {path,reason:"unsupported",tag:p[0]};
    }
  };
  return go(a,b,"root");
}
        ...
        if(args.length>=total){
          ...
          if(md?.kind==="ctor"&&md.induct===d.induct&&major.args.length===md.numParams+md.numFields){
            ...
            for(let i=0;i<d.numParams;i++){
              if(!closureSame(major.args[i],args[i])){
                paramsMatch=false;
                if(closureMode==="assume"){paramsMatch=true;closureScout.assumedParamMatches++;}
                break;
              }
            }
            if(paramsMatch){
          ...
          if(closureScout.stuckRecs.length<12){
            closureScout.stuckRecs.push({
              ...
              paramDiff:Array.from({length:d.numParams},(_,i)=>closureDiff(major.args[i],args[i])),
...
function install(enabled){
  ...
  proto.run=function(...args){
    ...
    closureScout={attempted:0,completed:0,failed:0,last:null,stuckRecs:[],assumedParamMatches:0};
...
install(false);
console.log("KERNEL_RG_CLOSURE_PARALLEL_V6 "+JSON.stringify({closureMode,arena_sha256:sha,result,closureScout}));
