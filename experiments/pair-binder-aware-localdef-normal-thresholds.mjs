...
const thresholds=[];
for(const budget of [1000000,1500000,2000000,3000000]){
  thresholds.push(evaluate(budget,true,focusRows));
}
install(false);
console.log("PAIR_BINDER_AWARE_LOCALDEF_NORMAL_THRESHOLDS "+JSON.stringify(thresholds));

const best=thresholds[thresholds.length-1];
if(best.wrong||!best.results.some(r=>r.status==="ACCEPT")){
  console.log("PAIR_BINDER_AWARE_LOCALDEF_NORMAL_STOP "+JSON.stringify({reason:"not-closed-through-3m",thresholds}));
  process.exit(0);
}
...
const report={arena_sha256:sha,budget:1000000,thresholds,
 ...
 claim_boundary:"Binder-aware LocalDef normalization repair. The retained iterative normalizer now carries the exact local context on each work frame: lambda/Pi domains use the current context and bodies use that context extended by the exact binder type. LocalDef WHNF is therefore never allowed to unfold a de-Bruijn variable that is bound by a lambda/Pi as though it were an outer local definition. Reduction, conversion, proof irrelevance and acceptance rules are unchanged; this repairs execution context propagation only."
};
mkdirSync(new URL("../genesis/evidence/",import.meta.url),{recursive:true});
writeFileSync(new URL("../genesis/evidence/pair-binder-aware-localdef-normal.json",import.meta.url),JSON.stringify(report,null,2)+"\n");
console.log("PAIR_BINDER_AWARE_LOCALDEF_NORMAL "+JSON.stringify(report));
