...
const proto=K.Kernel.prototype, fallback=proto.equal;
...
function rebuild(k,head,args){
  let out=head; for(const a of args) out=k.make("app",out,a); return out;
}
  ...
  proto.equal=function(a,b,ctx=[]){
    ...
    if(depth===0){
      ...
      try{
        ...
        this._lazyDeltaDepth=0;this.budget=snap.budget;return;
      }catch(e){
        this._lazyDeltaDepth=0;this.budget=snap.budget;
...
for(const cap of [null,300000,400000,500000,650000,800000]){
  install(cap);
  for(const row of rows){
    const r=K.checkExport(row.input,caps,1_000_000);
    console.log("LAZY_DELTA_CAP "+JSON.stringify({
      cap:cap??"baseline",name:row.name,expected:row.expected,status:r.status,
      reason:r.reason,steps:r.steps??null,constructed:r.constructed??null,
