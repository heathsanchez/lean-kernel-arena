...
function eqCanon(k,e){
  if(!Array.isArray(e))return e;
  k.__eqCanonWeak??=new WeakMap();k.__eqCanonRoot??=new Map();
  const old=k.__eqCanonWeak.get(e);if(old!==undefined)return old;
  const xs=new Array(e.length);
  for(let i=0;i<e.length;i++) xs[i]=Array.isArray(e[i])?eqCanon(k,e[i]):e[i];
  let node=k.__eqCanonRoot;
  for(const x of xs){
    let next=node.get(x);if(!(next instanceof Map)){next=new Map();node.set(x,next);}node=next;
  }
  let r=node.get(EQ_END);
  if(r===undefined){r=xs;node.set(EQ_END,r);}
  k.__eqCanonWeak.set(e,r);k.__eqCanonWeak.set(r,r);return r;
}
function semCtxKey(k,ctx){
  if(!ctx?.length)return "";
  return ctx.map(x=>{
    if(x?.__localDef===true) return "D:"+objectId(k,eqCanon(k,x.type))+":"+objectId(k,eqCanon(k,x.value));
    return "T:"+objectId(k,Array.isArray(x)?eqCanon(k,x):x);
  }).join("|");
}
function eqProgress(k,a,b){
  k.__eqProgress??=new WeakMap();
  a=eqCanon(k,a);b=eqCanon(k,b);
  let m=k.__eqProgress.get(a);if(!m){m=new WeakMap();k.__eqProgress.set(a,m);}
  let q=m.get(b);if(!q){q=new Set();m.set(b,q);}return q;
}
...
function install(enabled){
  proto.run=retainedRun;proto.equal=retainedEqual;proto.whnf=retainedWhnf;proto.infer=retainedInfer;proto.proofType=retainedProofType;
  if(!enabled)return;
  proto.run=function(...args){
    ...
    this.__closedInferCache=new WeakMap();this.__closedInferClosed=new WeakMap();this.__eqProgress=new WeakMap();this.__eqCanonWeak=new WeakMap();this.__eqCanonRoot=new Map();this.__skipPICalls=0;
  ...
  proto.proofType=function(e,ctx=[]){
    if((this.__skipPICalls??0)>0){
      this.__skipPICalls--;stats.piSkipped++;return null;
    }
    return retainedProofType.call(this,e,ctx);
  };
  ...
  proto.equal=function(a,b,ctx=[]){
    ...
    if(progress.has(progressKey)){
      stats.piCycleBreaks++;
      const prev=this.__skipPICalls??0;
      this.__skipPICalls=prev+2;
      try{return retainedEqual.call(this,a,b,ctx);}
      finally{this.__skipPICalls=prev;}
    }
...
const thresholds=[];
for(const budget of [1000000,1500000,2000000]){
  thresholds.push(evaluate(budget,true,focusRows));
}
install(false);
console.log("PAIR_STRUCTURAL_PI_CYCLE_GUARD_THRESHOLDS "+JSON.stringify(thresholds));


let full=null;
if(thresholds.some(x=>x.results?.[0]?.status==="ACCEPT")){
  const pyAll=[
    "import io,tarfile,json,sys",
    "data=sys.stdin.buffer.read();rows=[]",
    "with tarfile.open(fileobj=io.BytesIO(data),mode='r:gz') as a:",
  ...
  if(allRows.length!==188)throw new Error("Arena row count changed");
  full=evaluate(1000000,true,allRows);
  if(full.wrong)throw new Error("PI cycle guard wrong verdict");
}
console.log("PAIR_STRUCTURAL_PI_CYCLE_GUARD "+JSON.stringify({thresholds,full}));
process.exit(0);
