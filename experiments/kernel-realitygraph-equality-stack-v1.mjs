...
let rgLong=null;
function compactTerm(e){
  if(!Array.isArray(e))return {tag:typeof e,bytes:String(e).length,preview:String(e)};
  const raw=JSON.stringify(e);
  return {tag:e[0],bytes:raw.length,preview:raw.slice(0,2400)};
}
function captureEqStack(k,root,startStep,rootCtx){
  if(rgLong!==null)return;
  rgLong={
    startStep,crossStep:k.steps,declaration:k.currentDeclaration??null,
    root:compactTerm(root),
    rootContextDepth:rootCtx?.length??0,
    equalityStack:(k.__rgEqStack??[]).slice(-8).map(f=>({
      startStep:f.startStep,ctxDepth:f.ctx?.length??0,
      rootIsA:root===f.a,rootIsB:root===f.b,
      a:compactTerm(f.a),b:compactTerm(f.b)
    }))
  };
}
...
function deepLocalNormal(k,root){
  ...
  const rootCtx=k._activeCtx??[],rootStart=k.steps;
  const work=[{kind:"visit",e:root,ctx:rootCtx}],vals=[];
  const maybeCapture=()=>{if(rootStart<2000000&&k.steps>=2000000)captureEqStack(k,root,rootStart,rootCtx);};
  while(work.length){
    maybeCapture();
    const f=work.pop();
    if(f.kind==="build"){
...
function install(enabled){
  ...
  proto.equal=function(a,b,ctx=[]){
    this.__rgEqStack??=[];
    const __rgFrame={a,b,ctx,startStep:this.steps};
    this.__rgEqStack.push(__rgFrame);
    try{
...
install(false);
console.log("KERNEL_RG_EQUALITY_STACK "+JSON.stringify({arena_sha256:sha,result,rgLong}));
