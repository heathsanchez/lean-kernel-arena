...
const callerDiag={stack:[],counts:new Map(),origin:null};
function rawShape(e){
...
function termSnapshot(e){
  let json=null;
  try{json=JSON.stringify(e).slice(0,1800);}catch{}
  return {...rawShape(e),json};
}
function ctxSnapshot(ctx){
  return (ctx??[]).map((x,i)=>({
    slot:i,debruijn:(ctx?.length??0)-1-i,
    localDef:x?.__localDef===true,
    type:termSnapshot(x?.__localDef===true?x.type:x),
    value:x?.__localDef===true?termSnapshot(x.value):null
  }));
}
...
function tailRecord(k,e){
  ...
  const caller=callerDiag.stack.length?callerDiag.stack.map(f=>f.name).join(">"):"<none>";
  callerDiag.counts.set(caller,(callerDiag.counts.get(caller)??0)+1);
  if(callerDiag.origin===null){
    callerDiag.origin={
      atStep:k.steps,declaration:k.currentDeclaration??null,activeCtx:ctxSnapshot(k._activeCtx??[]),
      frames:callerDiag.stack.map(f=>({
        name:f.name,
        args:(f.args??[]).slice(0,3).map((a,i)=>i===2&&Array.isArray(a)&&a.every?.(()=>true)?{ctxLength:a.length}:termSnapshot(a))
      }))
    };
  }
...
function wrapCallers(){
  ...
  for(const name of names){
    ...
    proto[name]=function(...args){
      callerDiag.stack.push({name,args});
...
function evaluate(budget,enabled,rows){
  install(enabled);
  callerDiag.stack=[];callerDiag.counts=new Map();callerDiag.origin=null;
  ...
  return {budget,enabled,wrong,totalSteps,totalConstructed,elapsed_ms:Date.now()-t0,stats:d,results,whnfTail:tailSummary(),callers,loopOrigin:callerDiag.origin};
}
const thresholds=[];
for(const budget of [2050000]){
  thresholds.push(evaluate(budget,true,focusRows));
}
install(false);
console.log("PAIR_WHNF_LOOP_OBLIGATION_THRESHOLDS "+JSON.stringify(thresholds));

console.log("PAIR_WHNF_LOOP_OBLIGATION "+JSON.stringify(thresholds));
process.exit(0);
