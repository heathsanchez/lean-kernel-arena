...
function deepLocalNormal(k,root){
  ...
  const key=deepNormCtxKey(k,k._activeCtx??[]);
  ...
  while(work.length){
    ...
    const requested=f.e,slot=deepNormSlot(k,requested,key);
    if(slot?.has()){stats.deepNormalHits++;vals.push(slot.get());continue;}
    k.tick();
    const e=k.whnf(requested);
    if(e!==requested){
...
install(false);
console.log("PAIR_DEEP_LOCALDEF_NORMAL_THRESHOLDS "+JSON.stringify(thresholds));

const at1m=thresholds[0];
if(at1m.wrong||!at1m.results.some(r=>r.status==="ACCEPT")){
  console.log("PAIR_DEEP_LOCALDEF_NORMAL_STOP "+JSON.stringify({reason:"not-closed-at-1m",thresholds}));
  process.exit(0);
}
...
const report={arena_sha256:sha,budget:1000000,thresholds,
 ...
 claim_boundary:"Execution-only LocalDef normalization cache-key repair inside the retained iterative normalizer. Normalization algorithm and WHNF semantics are unchanged; exact expression identity is retained, while transient LocalDef wrapper identity is replaced by exact (type,value) identities in subtree cache keys. Only completed normalization consequences are reused. No semantic rule is added."
};
mkdirSync(new URL("../genesis/evidence/",import.meta.url),{recursive:true});
writeFileSync(new URL("../genesis/evidence/pair-deep-localdef-normal.json",import.meta.url),JSON.stringify(report,null,2)+"\n");
console.log("PAIR_DEEP_LOCALDEF_NORMAL "+JSON.stringify(report));
