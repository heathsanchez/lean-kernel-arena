...
const stats={eqHits:0,eqStores:0,earlyPi:0,frontiers:0,projectionEligible:0,projectionSuccess:0,argChecks:0,whnfQueries:0,whnfHits:0,whnfStores:0,whnfCanonNodes:0,whnfLocalBypass:0,closedInferQueries:0,closedInferEligible:0,closedInferHits:0,closedInferStores:0,closedChecks:0,normalTailCalls:0,normalTailReturns:0,normalTailRoots:0,normalTailNested:0,normalMaxDepth:0};
...
const normalTail={ids:new WeakMap(),next:1,records:new Map(),stepBuckets:new Map(),rootStarts:new Map()};
function rawShape(e){
...
function normalRecord(k,e,depth){
  if(k.localDefs!==true || k.steps<2000000 || !Array.isArray(e))return;
  stats.normalTailCalls++;
  if(depth===0)stats.normalTailRoots++;else stats.normalTailNested++;
  stats.normalMaxDepth=Math.max(stats.normalMaxDepth,depth);
  let id=normalTail.ids.get(e);
  if(id===undefined){
    id=normalTail.next++;normalTail.ids.set(e,id);
    normalTail.records.set(id,{id,count:0,returns:0,minDepth:depth,maxDepth:depth,...rawShape(e),
      firstStep:k.steps,lastStep:k.steps,firstDeclaration:k.currentDeclaration??null,lastDeclaration:k.currentDeclaration??null});
  }
  const r=normalTail.records.get(id);r.count++;r.minDepth=Math.min(r.minDepth,depth);r.maxDepth=Math.max(r.maxDepth,depth);
  r.lastStep=k.steps;r.lastDeclaration=k.currentDeclaration??null;
  const b=Math.floor(k.steps/100000)*100000;normalTail.stepBuckets.set(b,(normalTail.stepBuckets.get(b)??0)+1);
  return id;
}
function normalReturn(id){
  if(id===undefined)return;const r=normalTail.records.get(id);if(r){r.returns++;stats.normalTailReturns++;}
}
function normalSummary(){
  return {calls:stats.normalTailCalls,returns:stats.normalTailReturns,roots:stats.normalTailRoots,nested:stats.normalTailNested,
    maxDepth:stats.normalMaxDepth,unique:normalTail.records.size,
    topExact:[...normalTail.records.values()].sort((a,b)=>b.count-a.count).slice(0,50),
    stepBuckets:[...normalTail.stepBuckets.entries()].sort((a,b)=>a[0]-b[0])};
}
function install(enabled){
  ...
  proto.run=function(...args){
    ...
    normalTail.ids=new WeakMap();normalTail.next=1;normalTail.records=new Map();normalTail.stepBuckets=new Map();this.__normalDepth=0;
  ...
  proto.normal=function(e){
    if(this.localDefs!==true || !Array.isArray(e))return retainedNormal.call(this,e);
    const depth=this.__normalDepth??0, id=normalRecord(this,e,depth);
    this.__normalDepth=depth+1;
    try{
      const out=retainedNormal.call(this,e);
      normalReturn(id);return out;
    }finally{this.__normalDepth=depth;}
  };
...
function evaluate(budget,enabled,rows){
  ...
  return {budget,enabled,wrong,totalSteps,totalConstructed,elapsed_ms:Date.now()-t0,stats:d,results,normalTail:normalSummary()};
}
const thresholds=[];
...
install(false);
console.log("PAIR_NORMAL_TAIL_SHAPE_THRESHOLDS "+JSON.stringify(thresholds));

console.log("PAIR_NORMAL_TAIL_SHAPE "+JSON.stringify(thresholds));
process.exit(0);
