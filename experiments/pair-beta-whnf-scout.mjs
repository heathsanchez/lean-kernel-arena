...
function recordFallbackDecl(k,head){
  const decl=k.currentDeclaration??"<none>";
  const key=head+" @ "+decl;
  bump(fallbackDecls,key);
}
...
function shortShape(e){
  if(!Array.isArray(e))return typeof e+":"+String(e);
  const sp=rawSpine(e);
  if(sp.args.length&&sp.head?.[0]==="const")return "app:"+sp.head[1]+":"+sp.args.length;
  if(sp.args.length)return "app:"+(sp.head?.[0]??typeof sp.head)+":"+sp.args.length;
...
function countHead(e){
  ...
  switch(sp.head[1]){
    case DECIDE:
      stats.headDecide++;
      if(decideSamples.length<12)decideSamples.push({
        arity:sp.args.length,
        proposition:sp.args[0]?shortShape(sp.args[0]):null,
        instance:sp.args[1]?shortShape(sp.args[1]):null
      });
      break;
...
function install(enabled){
  ...
  proto.whnf=function(e){
    ...
    if(sp.args.length<2){
      stats.betaFallbacks++;
      bump(betaFallbackHeads,shortShape(sp.head));recordFallbackDecl(this,shortShape(sp.head));
      return structuralWhnf(this,e);
    }
