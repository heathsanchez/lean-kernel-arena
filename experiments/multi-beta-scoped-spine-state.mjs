...
console.log("MULTI_BETA_SCOPED_SPINE_STATE_FOCUS "+JSON.stringify({focus}));
if(focus.wrong)throw new Error("focus wrong");
if(focus.results[0]?.status!=="ACCEPT"){
  install(false);
  console.log("MULTI_BETA_SCOPED_SPINE_STATE_STOP "+JSON.stringify({reason:"beta-not-closed",focus}));
  process.exit(0);
}
...
if(candidate.wrong||baseline.wrong){
  ...
  console.log("MULTI_BETA_SCOPED_SPINE_STATE_WRONG "+JSON.stringify(wrongCases));
}
...
const summary={arena_sha256:sha,budget:1_000_000,focus,
 ...
 claim_boundary:"Exact execution-order change only. For one detected chain of two-or-more consecutive raw beta redexes, arguments are materialized once with exact de-Bruijn capture counts and the resulting term is reduced to WHNF on one explicit local continuation carrying application spines as head+argument state across definition unfolding, beta, recursor, and projection reductions. The continuation is disabled immediately when that WHNF request returns. No later unrelated WHNF call is captured; no conversion, typing, or reduction law is added."
};
mkdirSync(new URL("../genesis/evidence/",import.meta.url),{recursive:true});
writeFileSync(new URL("../genesis/evidence/multi-beta-scoped-spine-state.json",import.meta.url),JSON.stringify(summary,null,2)+"\n");
console.log("MULTI_BETA_SCOPED_SPINE_STATE "+JSON.stringify(summary));
