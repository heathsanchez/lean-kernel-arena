...
console.log("MULTI_BETA_EXACT_SCOPED_SPINE_FOCUS "+JSON.stringify({focus}));
if(focus.wrong)throw new Error("focus wrong");
if(focus.results[0]?.status!=="ACCEPT"){
  install(false);
  console.log("MULTI_BETA_EXACT_SCOPED_SPINE_STOP "+JSON.stringify({reason:"beta-not-closed",focus}));
  process.exit(0);
}
...
if(candidate.wrong||baseline.wrong){
  ...
  console.log("MULTI_BETA_EXACT_SCOPED_SPINE_WRONG "+JSON.stringify(wrongCases));
}
...
const summary={arena_sha256:sha,budget:1_000_000,focus,
 ...
 claim_boundary:"Exact execution-order change only, scoped strictly to one WHNF request whose raw syntax exposes two-or-more consecutive app(lam,arg) beta redexes. Arguments are materialized once with exact de-Bruijn capture counts and that result alone is reduced on one explicit application/recursor/projection continuation. The wrapper does not intercept generic stack fallback or later unrelated WHNF calls. No conversion, typing, or reduction law is added."
};
mkdirSync(new URL("../genesis/evidence/",import.meta.url),{recursive:true});
writeFileSync(new URL("../genesis/evidence/multi-beta-exact-scoped-spine.json",import.meta.url),JSON.stringify(summary,null,2)+"\n");
console.log("MULTI_BETA_EXACT_SCOPED_SPINE "+JSON.stringify(summary));
