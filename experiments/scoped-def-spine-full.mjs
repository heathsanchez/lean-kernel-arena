...
function install(candidate){
  ...
  proto.equal=function(a,b,ctx=[]){
    if(this._defSpineSpeculating||this.localDefs)return retained.call(this,a,b,ctx);
...
const baseline=evaluate("baseline",false),candidate=evaluate("scoped-def-spine",true);
proto.equal=retained;
...
const summary={arena_sha256:sha,budget,speculation_cap:CAP,
  ...
  claim_boundary:"Single-level exact congruence probe only for applications with the same exact reducible definition head. Argument equality is delegated to the retained converter under a 250k speculative step cap; failed probes restore semantic budget/frontier."};
mkdirSync(new URL("../genesis/evidence/",import.meta.url),{recursive:true});
writeFileSync(new URL("../genesis/evidence/scoped-def-spine-full.json",import.meta.url),JSON.stringify({summary,baseline,candidate},null,2));
console.log("SCOPED_DEF_SPINE_FULL "+JSON.stringify(summary));
