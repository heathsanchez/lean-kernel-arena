...
install(false);
console.log("PAIR_CLOSED_INFER_THRESHOLDS "+JSON.stringify(thresholds));

const at1m=thresholds[0];
if(at1m.wrong||!at1m.results.some(r=>r.status==="ACCEPT")){
  console.log("PAIR_CLOSED_INFER_STOP "+JSON.stringify({reason:"not-closed-at-1m",thresholds}));
  process.exit(0);
}
...
const report={arena_sha256:sha,budget:1000000,thresholds,
 ...
 claim_boundary:"Composition of execution-only consequences: exact structural ordinary-kernel WHNF reuse, exact successful inference reuse for de-Bruijn-closed expressions independent of ambient local context, early Pi congruence, exact equality memory, and positive recursor-projection recovery. Closedness is structural and exact; only successful retained inference results are cached. No typing, equality or reduction law is added."
};
mkdirSync(new URL("../genesis/evidence/",import.meta.url),{recursive:true});
writeFileSync(new URL("../genesis/evidence/pair-closed-infer.json",import.meta.url),JSON.stringify(report,null,2)+"\n");
console.log("PAIR_CLOSED_INFER "+JSON.stringify(report));
