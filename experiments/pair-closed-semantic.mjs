...
const proto=K.Kernel.prototype,retainedRun=proto.run,retainedEqual=proto.equal,retainedWhnf=proto.whnf,retainedInfer=proto.infer,retainedSortOf=proto.sortOf,retainedProofType=proto.proofType;
const stats={eqHits:0,eqStores:0,earlyPi:0,frontiers:0,projectionEligible:0,projectionSuccess:0,argChecks:0,whnfQueries:0,whnfHits:0,whnfStores:0,whnfCanonNodes:0,whnfLocalBypass:0,closedChecks:0,closedInferQueries:0,closedInferEligible:0,closedInferHits:0,closedInferStores:0,closedSortQueries:0,closedSortHits:0,closedSortStores:0,closedProofQueries:0,closedProofHits:0,closedProofStores:0,closedEqQueries:0,closedEqEligible:0,closedEqHits:0,closedEqStores:0};
...
function closedRoot(k,e){
  ...
  k.__closedSemanticClosed??=new WeakMap();
  const old=k.__closedSemanticClosed.get(e);
  ...
  const out=walk(e,0);
  k.__closedSemanticClosed.set(e,out);
  return out;
}
function closedEqSlot(k,a,b){
  k.__closedSemanticEq??=new WeakMap();
  let byA=k.__closedSemanticEq.get(a);
  if(!byA){byA=new WeakMap();k.__closedSemanticEq.set(a,byA);}
  return {byA,hit:byA.has(b)};
}
function install(enabled){
  proto.run=retainedRun;proto.equal=retainedEqual;proto.whnf=retainedWhnf;proto.infer=retainedInfer;proto.sortOf=retainedSortOf;proto.proofType=retainedProofType;
  if(!enabled)return;
  proto.run=function(...args){
    ...
    this.__closedSemanticClosed=new WeakMap();this.__closedSemanticInfer=new WeakMap();this.__closedSemanticSort=new WeakMap();this.__closedSemanticProof=new WeakMap();this.__closedSemanticEq=new WeakMap();
  ...
  proto.infer=function(e,ctx=[]){
    ...
    this.__closedSemanticInfer??=new WeakMap();
    if(this.__closedSemanticInfer.has(e)){stats.closedInferHits++;return this.__closedSemanticInfer.get(e);}
    const out=retainedInfer.call(this,e,ctx);
    this.__closedSemanticInfer.set(e,out);stats.closedInferStores++;
    return out;
  };
  proto.sortOf=function(e,ctx=[]){
    if(!Array.isArray(e))return retainedSortOf.call(this,e,ctx);
    stats.closedSortQueries++;
    if(!closedRoot(this,e))return retainedSortOf.call(this,e,ctx);
    this.__closedSemanticSort??=new WeakMap();
    if(this.__closedSemanticSort.has(e)){stats.closedSortHits++;return this.__closedSemanticSort.get(e);}
    const out=retainedSortOf.call(this,e,ctx);
    this.__closedSemanticSort.set(e,out);stats.closedSortStores++;
    return out;
  };
  proto.proofType=function(e,ctx=[]){
    if(!Array.isArray(e))return retainedProofType.call(this,e,ctx);
    stats.closedProofQueries++;
    if(!closedRoot(this,e))return retainedProofType.call(this,e,ctx);
    this.__closedSemanticProof??=new WeakMap();
    if(this.__closedSemanticProof.has(e)){stats.closedProofHits++;return this.__closedSemanticProof.get(e);}
    const out=retainedProofType.call(this,e,ctx);
    this.__closedSemanticProof.set(e,out);stats.closedProofStores++;
    return out;
  };
  proto.equal=function(a,b,ctx=[]){
    if(!Array.isArray(a)||!Array.isArray(b))return retainedEqual.call(this,a,b,ctx);
    stats.closedEqQueries++;
    const bothClosed=closedRoot(this,a)&&closedRoot(this,b);
    if(bothClosed){
      stats.closedEqEligible++;
      const ce=closedEqSlot(this,a,b);
      if(ce.hit){stats.closedEqHits++;return;}
      try{
        const out=retainedEqual.call(this,a,b,ctx);
        ce.byA.set(b,true);closedEqSlot(this,b,a).byA.set(a,true);stats.closedEqStores++;
        return out;
      }catch(err){
        // The pair-specific conversion recovery below is still allowed to act.
        if(!(err instanceof Stop) || err.status!==UNKNOWN || err.message!=="conversion-frontier") throw err;
      }
    }
...
install(false);
console.log("PAIR_CLOSED_SEMANTIC_THRESHOLDS "+JSON.stringify(thresholds));

const at1m=thresholds[0];
if(at1m.wrong||!at1m.results.some(r=>r.status==="ACCEPT")){
  console.log("PAIR_CLOSED_SEMANTIC_STOP "+JSON.stringify({reason:"not-closed-at-1m",thresholds}));
  process.exit(0);
}
...
const report={arena_sha256:sha,budget:1000000,thresholds,
 ...
 claim_boundary:"Execution-only closed semantic consequence bank. De-Bruijn-closed expressions are structurally proven independent of ambient local context, so completed infer, sortOf, proofType (including null), and successful equality consequences are reused without context keys. Pair frontier recovery and ordinary structural WHNF remain as independently tested execution policies. No typing, equality or reduction law is added."
};
mkdirSync(new URL("../genesis/evidence/",import.meta.url),{recursive:true});
writeFileSync(new URL("../genesis/evidence/pair-closed-semantic.json",import.meta.url),JSON.stringify(report,null,2)+"\n");
console.log("PAIR_CLOSED_SEMANTIC "+JSON.stringify(report));
