...
const baseline=evaluate("baseline",false),candidate=evaluate("lazy-delta-650k",true);
...
const summary={arena_sha256:sha,budget,lazy_delta_cap:CAP,
  ...
  claim_boundary:"Execution-order consequence only. Same rigid inductive applications start one 650k transactional congruence. Inside it, exact same-head spines use congruence; differing heads take shallow delta/iota steps, preferring a folded definition over evaluating an exposed recursor and stopping as soon as heads align. Failure rolls back to the retained converter."};
mkdirSync(new URL("../genesis/evidence/",import.meta.url),{recursive:true});
writeFileSync(new URL("../genesis/evidence/lazy-delta-full.json",import.meta.url),JSON.stringify({summary,baseline,candidate},null,2));
console.log("LAZY_DELTA_FULL "+JSON.stringify(summary));
