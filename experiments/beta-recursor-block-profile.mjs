...
const stats={queries:0,chains:0,redexes:0,maxChain:0,materializedNodes:0,varSubstitutions:0,
  iterativeCalls:0,flattenApps:0,rebuildApps:0,defUnfolds:0,recFrames:0,recReductions:0,recBlocked:0,blockReasons:Object.create(null),blockSamples:[]};
...
function iterativeRecWhnf(k,root){
  ...
  function resumeRec(fr,major){
    ...
    let reason=null;
    if(md?.kind!=="ctor") reason="major-not-ctor:"+(md?.kind??mh?.[0]??typeof mh);
    else if(md.induct!==rd.induct) reason="induct-mismatch";
    else if(margs.length!==md.numParams+md.numFields)
      reason="ctor-arity:"+margs.length+"/"+(md.numParams+md.numFields);
    else {
      let paramsMatch=true;
      for(let i=0;i<rd.numParams;i++){
        if(!k.same(margs[i],rargs[i])){paramsMatch=false;break;}
      }
      if(!paramsMatch) reason="params-mismatch";
      else {
        const rule=rd.rules.find(rr=>rr.ctor===md.name);
        if(!rule) reason="no-rule";
        else {
          stats.recReductions++;
    ...
    if(stats.blockSamples.length<20) stats.blockSamples.push({
      reason,
      recHead:rh?.[1]??null,
      recInduct:rd?.induct??null,
      recTotal:total,
      recArgs:rargs.length,
      majorTag:mh?.[0]??null,
      majorHead:mh?.[1]??null,
      majorDeclKind:md?.kind??null,
      majorInduct:md?.induct??null,
      majorArgs:margs.length,
      majorExpected:md?.kind==="ctor"?(md.numParams+md.numFields):null
    });
  ...
  for(;;){
    ...
    if(head[0]==="let"){
      ...
      state=flatten(rebuild(k.substitute(head[3],head[2]),args));
      continue;
    }
    ...
    if(head[0]==="nat"){
      ...
      state=flatten(rebuild(lit,args));
      continue;
    }
    ...
    if(head[0]==="lam"){
      ...
      if(args.length){
        k.need("reduction");
        state=flatten(rebuild(k.substitute(head[2],args[0]),args.slice(1)));
        continue;
      }
    }
...
console.log("MULTI_BETA_STICKY_ITERATIVE_FOCUS "+JSON.stringify({focus}));
if(focus.wrong)throw new Error("focus wrong");
install(false);
console.log("BETA_RECURSOR_BLOCK_PROFILE "+JSON.stringify({focus,blockReasons:stats.blockReasons,blockSamples:stats.blockSamples}));
process.exit(0);
...
const summary={arena_sha256:sha,budget:1_000_000,focus,
 ...
 claim_boundary:"Exact execution-order change for two-or-more consecutive raw app(lam,arg) beta redexes. Once such a chain is observed in one kernel run, subsequent WHNF requests in that same run stay on the explicit iterative recursor/application continuation rather than handing back to host recursion. Arguments are materialized once with exact de-Bruijn capture counts. No conversion, typing, or reduction law is added."
};
mkdirSync(new URL("../genesis/evidence/",import.meta.url),{recursive:true});
writeFileSync(new URL("../genesis/evidence/multi-beta-sticky-iterative.json",import.meta.url),JSON.stringify(summary,null,2)+"\n");
console.log("MULTI_BETA_STICKY_ITERATIVE "+JSON.stringify(summary));
