...
const stats={eqHits:0,eqStores:0,earlyPi:0,frontiers:0,projectionEligible:0,projectionSuccess:0,argChecks:0,whnfQueries:0,whnfHits:0,whnfStores:0,whnfCanonNodes:0,whnfLocalBypass:0,closedInferQueries:0,closedInferEligible:0,closedInferHits:0,closedInferStores:0,closedChecks:0,nonclosedInfer:0};
...
const supportDiag={ids:new WeakMap(),next:1,terms:new Map(),ctxIds:new WeakMap(),ctxNext:1};
function ctxObjId(x){
  if(x===null || (typeof x!=="object"&&typeof x!=="function"))return typeof x+":"+String(x);
  let id=supportDiag.ctxIds.get(x);
  if(id!==undefined)return "o"+id;
  id=supportDiag.ctxNext++;supportDiag.ctxIds.set(x,id);return "o"+id;
}
function freeWidth(root){
  ...
  function go(e,depth){
    ...
    let w=0;
    switch(e[0]){
      case "sort":case "const":case "nat":case "strlit":w=0;break;
      case "var":w=e[1]<depth?0:(e[1]-depth+1);break;
      case "pi":case "lam":w=Math.max(go(e[1],depth),go(e[2],depth+1));break;
      case "app":w=Math.max(go(e[1],depth),go(e[2],depth));break;
      case "proj":w=go(e[3],depth);break;
      case "let":w=Math.max(go(e[1],depth),go(e[2],depth),go(e[3],depth+1));break;
      default:w=999999;
    }
    byDepth.set(depth,w);return w;
  }
  return go(root,0);
}
function supportRecord(e,ctx){
  if(!Array.isArray(e))return;
  let id=supportDiag.ids.get(e);
  if(id===undefined){
    id=supportDiag.next++;supportDiag.ids.set(e,id);
    const w=freeWidth(e);
    supportDiag.terms.set(id,{id,count:0,width:w,tag:e[0],ctxLens:new Map(),suffixes:new Set(),fullCtx:new Set()});
  }
  const rec=supportDiag.terms.get(id);rec.count++;
  rec.ctxLens.set(ctx.length,(rec.ctxLens.get(ctx.length)??0)+1);
  const w=Math.min(rec.width,ctx.length);
  const suffix=ctx.slice(ctx.length-w).map(ctxObjId).join("|");
  const full=ctx.map(ctxObjId).join("|");
  rec.suffixes.add(suffix);rec.fullCtx.add(full);
}
function supportSummary(){
  return [...supportDiag.terms.values()]
    .sort((a,b)=>b.count-a.count)
    .slice(0,40)
    .map(r=>({
      id:r.id,count:r.count,width:r.width,tag:r.tag,
      ctxLens:[...r.ctxLens.entries()].sort((a,b)=>a[0]-b[0]),
      uniqueSuffixes:r.suffixes.size,uniqueFullContexts:r.fullCtx.size,
      compression:r.fullCtx.size?1-r.suffixes.size/r.fullCtx.size:0
    }));
}
function install(enabled){
  ...
  proto.run=function(...args){
    ...
    supportDiag.ids=new WeakMap();supportDiag.next=1;supportDiag.terms=new Map();supportDiag.ctxIds=new WeakMap();supportDiag.ctxNext=1;
  ...
  proto.infer=function(e,ctx=[]){
    ...
    if(!closedRoot(this,e)){
      stats.nonclosedInfer++;supportRecord(e,ctx);
...
function evaluate(budget,enabled,rows){
  ...
  return {budget,enabled,wrong,totalSteps,totalConstructed,elapsed_ms:Date.now()-t0,stats:d,results,supportSummary:supportSummary()};
}
const thresholds=[];
...
install(false);
console.log("PAIR_SUPPORT_SLICE_THRESHOLDS "+JSON.stringify(thresholds));

console.log("PAIR_SUPPORT_SLICE "+JSON.stringify(thresholds));
process.exit(0);
