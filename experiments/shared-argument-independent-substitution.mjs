...
const focusRows=rows.filter(r=>r.name.endsWith("good/perf/shared-subterm.ndjson"));
const focus=evalRows("focus",true,focusRows);
console.log("SHARED_ARG_INDEPENDENT_SUBST_FOCUS "+JSON.stringify({focus}));
if(focus.wrong)throw new Error("focus wrong");
if(focus.results[0]?.status!=="ACCEPT"){
  install(false);
  console.log("SHARED_ARG_INDEPENDENT_SUBST_STOP "+JSON.stringify({reason:"shared-not-closed",focus}));
  process.exit(0);
}
...
writeFileSync(new URL("../genesis/evidence/shared-argument-independent-substitution.json",import.meta.url),JSON.stringify(summary,null,2)+"\n");
console.log("SHARED_ARG_INDEPENDENT_SUBST "+JSON.stringify(summary));
