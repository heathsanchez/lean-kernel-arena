...
function semObjectId(k,x){
  if(x===null || (typeof x!=="object"&&typeof x!=="function")) return typeof x+":"+String(x);
  k.__ldSemIds??=new WeakMap();k.__ldSemNextId??=1;
  let id=k.__ldSemIds.get(x);if(id!==undefined)return "o"+id;
  id=k.__ldSemNextId++;k.__ldSemIds.set(x,id);return "o"+id;
}
function localDefCtxKey(k,ctx){
  stats.ldCtxKeys++;
  if(!ctx?.length)return "";
  ...
  for(let i=0;i<ctx.length;i++){
    ...
    if(x?.__localDef===true){
      stats.ldLocalEntries++;
      parts[i]="D:"+semObjectId(k,x.type)+":"+semObjectId(k,x.value);
    }else parts[i]="T:"+semObjectId(k,x);
  }
  return parts.join("|");
}
...
function install(enabled){
  ...
  proto.run=function(...args){
    ...
    this.__ldWhnfCache=new WeakMap();this.__ldSemIds=new WeakMap();this.__ldSemNextId=1;
    return retainedRun.apply(this,args);
  };
  proto.whnf=function(e){
    if(!Array.isArray(e)) return retainedWhnf.call(this,e);
    if(this.localDefs===true){
      stats.whnfLocalBypass++;stats.ldWhnfQueries++;
      this.__ldWhnfCache??=new WeakMap();
      let byCtx=this.__ldWhnfCache.get(e);
      if(!byCtx){byCtx=new Map();this.__ldWhnfCache.set(e,byCtx);}
      const key=localDefCtxKey(this,this._activeCtx??[]);
      if(byCtx.has(key)){stats.ldWhnfHits++;return byCtx.get(key);}
      const out=retainedWhnf.call(this,e);
      // Store only completed reductions; Stop/UNKNOWN/REJECT never populate the cache.
      byCtx.set(key,out);stats.ldWhnfStores++;
      return out;
    }
    stats.whnfQueries++;
...
install(false);
console.log("PAIR_CONTEXT_WHNF_THRESHOLDS "+JSON.stringify(thresholds));


let full=null;
if(thresholds[0]?.results?.[0]?.status==="ACCEPT"){
  const pyAll=[
    "import io,tarfile,json,sys",
    "data=sys.stdin.buffer.read();rows=[]",
    "with tarfile.open(fileobj=io.BytesIO(data),mode='r:gz') as a:",
  ...
  if(full.wrong)throw new Error("context-WHNF wrong verdict");
}
console.log("PAIR_CONTEXT_WHNF "+JSON.stringify({thresholds,full}));
process.exit(0);
