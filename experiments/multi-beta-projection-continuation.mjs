    ...
    while(frames.length){
      ...
      if(fr.kind==="proj"){
        ...
        if(ctor!==null&&mh?.[0]==="const"&&mh[1]===ctor){
          ...
          state=flatten(rebuild(margs[pos],fr.args));
          result=null;
          break;
        }
        stats.projBlocked++;
...
console.log("MULTI_BETA_PROJECTION_CONTINUATION_FOCUS "+JSON.stringify({focus}));
if(focus.wrong)throw new Error("focus wrong");
if(focus.results[0]?.status!=="ACCEPT"){
  install(false);
  console.log("MULTI_BETA_PROJECTION_CONTINUATION_STOP "+JSON.stringify({reason:"beta-not-closed",focus}));
  process.exit(0);
}
...
const summary={arena_sha256:sha,budget:1_000_000,focus,
 ...
 claim_boundary:"Exact execution-order change only. Two-or-more consecutive raw beta redexes are materialized once with exact de-Bruijn capture counts. Subsequent WHNF stays on one explicit continuation that suspends both recursors and projections until their major/object reaches WHNF, applying only the retained single-constructor projection rule and retained recursor rule. No conversion, typing, or reduction law is added."
};
mkdirSync(new URL("../genesis/evidence/",import.meta.url),{recursive:true});
writeFileSync(new URL("../genesis/evidence/multi-beta-projection-continuation.json",import.meta.url),JSON.stringify(summary,null,2)+"\n");
console.log("MULTI_BETA_PROJECTION_CONTINUATION "+JSON.stringify(summary));
