...
function install(enabled){
  ...
  proto.whnf=function(e){
    ...
    try{
      ...
      return iterativeRecWhnf(this,out);
    }finally{
...
console.log("MULTI_BETA_RECURSOR_FOCUS "+JSON.stringify({focus}));
if(focus.wrong)throw new Error("focus wrong");
if(focus.results[0]?.status!=="ACCEPT"){
  install(false);
  console.log("MULTI_BETA_RECURSOR_STOP "+JSON.stringify({reason:"beta-not-closed",focus}));
  process.exit(0);
}
...
const summary={arena_sha256:sha,budget:1_000_000,focus,
 ...
 claim_boundary:"Exact execution-order change for two-or-more consecutive raw app(lam,arg) beta redexes. Arguments are retained in one shared positional environment; the final body is materialized once with an explicit stack using exact de-Bruijn capture counts, then delegated to retained WHNF. No conversion, typing, or reduction law is added."
};
mkdirSync(new URL("../genesis/evidence/",import.meta.url),{recursive:true});
writeFileSync(new URL("../genesis/evidence/multi-beta-recursor.json",import.meta.url),JSON.stringify(summary,null,2)+"\n");
console.log("MULTI_BETA_RECURSOR "+JSON.stringify(summary));
