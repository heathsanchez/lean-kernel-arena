// Prospective separator: preserve a retained field-level REJECT from an exact
// same constructor application, but only for constructors of non-Prop
// inductives. Prop-valued constructors are excluded because proof irrelevance
// destroys constructor-argument injectivity as a definitional inequality rule.
...
function nonPropCtor(kernel,head){
  if(!Array.isArray(head)||head[0]!=="const") return false;
  const c=kernel.env.get(head[1]);
  if(c?.kind!=="ctor") return false;
  const ind=kernel.env.get(c.induct);
  return ind?.kind==="inductive" && ind.isProp===false;
}
function install(enabled){
  ...
  proto.equal=function(a,b,ctx=[]){
    if(this.localDefs || (this._rigidTypeSpineDepth??0)===0 || (this._nonPropCtorDepth??0)>0)
    ...
    if(!sameHead || !nonPropCtor(this,sa.head))
      return retained.call(this,a,b,ctx);
...
const baseline=evaluate("baseline",false),candidate=evaluate("nonprop-ctor-reject",true);
proto.equal=retained;
...
const summary={arena_sha256:sha,budget,speculation_cap:CAP,
  ...
  claim_boundary:"Negative execution consequence only inside an already-open rigid type-spine transaction. For exact same constructors of non-Prop inductives, a field-level REJECT from retained conversion propagates by constructor injectivity. Prop-valued inductives are excluded; UNKNOWN/host-stack rolls back."
};
mkdirSync(new URL("../genesis/evidence/",import.meta.url),{recursive:true});
writeFileSync(new URL("../genesis/evidence/nonprop-ctor-reject.json",import.meta.url),JSON.stringify({summary,baseline,candidate},null,2));
console.log("NONPROP_CTOR_REJECT "+JSON.stringify(summary));
