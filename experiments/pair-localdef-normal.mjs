...
const stats={eqHits:0,eqStores:0,earlyPi:0,frontiers:0,projectionEligible:0,projectionSuccess:0,argChecks:0,whnfQueries:0,whnfHits:0,whnfStores:0,whnfCanonNodes:0,whnfLocalBypass:0,localNormalQueries:0,localNormalHits:0,localNormalStores:0,localNormalCtxKeys:0,localNormalEntries:0};
...
function localNormObjectId(k,x){
  k.__localNormIds??=new WeakMap();k.__localNormNextId??=1;
  if(x===null || (typeof x!=="object"&&typeof x!=="function"))return typeof x+":"+String(x);
  let id=k.__localNormIds.get(x);
  if(id!==undefined)return "o"+id;
  id=k.__localNormNextId++;k.__localNormIds.set(x,id);return "o"+id;
}
function localNormCtxKey(k,ctx){
  stats.localNormalCtxKeys++;
  if(!ctx?.length)return "";
  const parts=new Array(ctx.length);
  for(let i=0;i<ctx.length;i++){
    const x=ctx[i];
    if(x?.__localDef===true){
      stats.localNormalEntries++;
      parts[i]="D:"+localNormObjectId(k,x.type)+":"+localNormObjectId(k,x.value);
    }else parts[i]="T:"+localNormObjectId(k,x);
  }
  return parts.join("|");
}
function install(enabled){
  ...
  proto.run=function(...args){
    ...
    this.__localNormIds=new WeakMap();this.__localNormNextId=1;this.__localNormCache=new WeakMap();
  ...
  proto.normal=function(e){
    ...
    stats.localNormalQueries++;
    this.__localNormCache??=new WeakMap();
    let byCtx=this.__localNormCache.get(e);
    if(!byCtx){byCtx=new Map();this.__localNormCache.set(e,byCtx);}
    const key=localNormCtxKey(this,this._activeCtx??[]);
    if(byCtx.has(key)){stats.localNormalHits++;return byCtx.get(key);}
    const out=retainedNormal.call(this,e);
    byCtx.set(key,out);stats.localNormalStores++;
    return out;
  };
...
install(false);
console.log("PAIR_LOCALDEF_NORMAL_THRESHOLDS "+JSON.stringify(thresholds));

const at1m=thresholds[0];
if(at1m.wrong||!at1m.results.some(r=>r.status==="ACCEPT")){
  console.log("PAIR_LOCALDEF_NORMAL_STOP "+JSON.stringify({reason:"not-closed-at-1m",thresholds}));
  process.exit(0);
}
...
const report={arena_sha256:sha,budget:1000000,thresholds,
 ...
 claim_boundary:"Execution-only LocalDef normalization reuse. Exact expression identity is retained; only the transient LocalDef wrapper identity in context keys is replaced by the exact identity pair (entry.type, entry.value), with binder order and ordinary binder identities preserved. Only successful retained normal results are cached. Pair conversion recovery and ordinary structural WHNF are unchanged. No semantic rule is added."
};
mkdirSync(new URL("../genesis/evidence/",import.meta.url),{recursive:true});
writeFileSync(new URL("../genesis/evidence/pair-localdef-normal.json",import.meta.url),JSON.stringify(report,null,2)+"\n");
console.log("PAIR_LOCALDEF_NORMAL "+JSON.stringify(report));
