...
function rootMetrics(root){
  const seen=new WeakSet(),stack=[{e:root,d:0}],tags={},depths={};
  let unique=0,maxDepth=0,edges=0;
  while(stack.length){
    const {e,d}=stack.pop();if(!Array.isArray(e))continue;
    maxDepth=Math.max(maxDepth,d);depths[d]=(depths[d]??0)+1;
    if(seen.has(e))continue;seen.add(e);unique++;
    tags[e[0]]=(tags[e[0]]??0)+1;
    for(let i=1;i<e.length;i++)if(Array.isArray(e[i])){edges++;stack.push({e:e[i],d:d+1});}
  }
  const raw=JSON.stringify(root);
  return {unique,maxDepth,edges,bytes:raw.length,tags,preview:raw.slice(0,5000)};
}
function ctxSummary(ctx){
  return {
    depth:ctx?.length??0,
    shape:(ctx??[]).map(x=>x?.__localDef===true?"D":"T").join(""),
    localDefs:(ctx??[]).filter(x=>x?.__localDef===true).length
  };
}
...
function deepLocalNormal(k,root){
  ...
  const rootCtx=k._activeCtx??[],rootStartStep=k.steps,rootDecl=k.currentDeclaration??null;
  const work=[{kind:"visit",e:root,ctx:rootCtx}],vals=[];
  const maybeCapture=()=>{
    if(longRoot===null && rootStartStep<2000000 && k.steps>=2000000){
      longRoot={startStep:rootStartStep,crossStep:k.steps,declaration:rootDecl,
        context:ctxSummary(rootCtx),freeSupport:freeSupport(k,root),metrics:rootMetrics(root),
        workDepth:work.length,valsDepth:vals.length};
    }
  };
  while(work.length){
    maybeCapture();
...
const result=evaluate(2100000,true,focusRows);
install(false);
console.log("KERNEL_RG_LONG_NORMAL_ROOT "+JSON.stringify({arena_sha256:sha,result,longRoot}));
