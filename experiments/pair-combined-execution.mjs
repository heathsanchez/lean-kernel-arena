...
const stats={eqHits:0,eqStores:0,earlyPi:0,frontiers:0,projectionEligible:0,projectionSuccess:0,argChecks:0};
...
install(false);
console.log("PAIR_COMBINED_THRESHOLDS "+JSON.stringify(thresholds));

const at1m=thresholds[0];
if(at1m.wrong||!at1m.results.some(r=>r.status==="ACCEPT")){
  console.log("PAIR_COMBINED_STOP "+JSON.stringify({reason:"not-closed-at-1m",thresholds}));
  process.exit(0);
}
...
const report={arena_sha256:sha,budget:1000000,thresholds,
 ...
 claim_boundary:"Composition of previously lawful execution-only consequences: early raw Pi congruence in the ordinary kernel, exact successful equality memory under exact context identity, and positive recursor-projection congruence recovery after conversion-frontier. No new equality law is introduced."
};
mkdirSync(new URL("../genesis/evidence/",import.meta.url),{recursive:true});
writeFileSync(new URL("../genesis/evidence/pair-combined-execution.json",import.meta.url),JSON.stringify(report,null,2)+"\n");
console.log("PAIR_COMBINED "+JSON.stringify(report));
