...
const thresholds=[];
for(const budget of [1000000,1500000,2000000,2100000]){
  thresholds.push(evaluate(budget,true,focusRows));
}
install(false);
console.log("PAIR_SUPPORT_SLICED_LOCALDEF_NORMAL_THRESHOLDS "+JSON.stringify(thresholds));

const best=thresholds[thresholds.length-1];
if(best.wrong||!best.results.some(r=>r.status==="ACCEPT")){
  console.log("PAIR_SUPPORT_SLICED_LOCALDEF_NORMAL_STOP "+JSON.stringify({reason:"not-closed-through-2.1m",thresholds}));
  process.exit(0);
}

console.log("PAIR_SUPPORT_SLICED_LOCALDEF_NORMAL_HIT "+JSON.stringify({thresholds}));
