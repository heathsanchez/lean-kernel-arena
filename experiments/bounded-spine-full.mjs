...
const baseline=evaluate("baseline",false),candidate=evaluate("bounded-spine-250k",true);
proto.equal=retained;
...
const summary={arena_sha256:sha,budget,speculation_cap:CAP,
  ...
  claim_boundary:"Exact same-head congruence is attempted cheapest-first under a 250k speculative semantic-step cap. Failed speculation restores budget and conversion frontier. REJECT is propagated only for an exact rigid constructor head."};
mkdirSync(new URL("../genesis/evidence/",import.meta.url),{recursive:true});
writeFileSync(new URL("../genesis/evidence/bounded-spine-full.json",import.meta.url),JSON.stringify({summary,baseline,candidate},null,2));
console.log("BOUNDED_SPINE_FULL "+JSON.stringify(summary));
