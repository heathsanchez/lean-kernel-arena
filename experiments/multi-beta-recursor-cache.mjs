...
console.log("MULTI_BETA_RECURSOR_CACHE_FOCUS "+JSON.stringify({focus}));
if(focus.wrong)throw new Error("focus wrong");
if(focus.results[0]?.status!=="ACCEPT"){
  install(false);
  console.log("MULTI_BETA_RECURSOR_CACHE_STOP "+JSON.stringify({reason:"beta-not-closed",focus}));
  process.exit(0);
}
...
writeFileSync(new URL("../genesis/evidence/multi-beta-recursor-cache.json",import.meta.url),JSON.stringify(summary,null,2)+"\n");
console.log("MULTI_BETA_RECURSOR_CACHE "+JSON.stringify(summary));
