...
function install(enabled){
  ...
  proto.whnf=function(e){
    if(this.localDefs===true || !Array.isArray(e)){
      if(this.localDefs===true){stats.whnfLocalBypass++;tailRecord(this,e);}
...
function evaluate(budget,enabled,rows){
  ...
  return {budget,enabled,wrong,totalSteps,totalConstructed,elapsed_ms:Date.now()-t0,stats:d,results,whnfTail:tailSummary()};
}
const thresholds=[];
for(const budget of [4000000]){
  thresholds.push(evaluate(budget,true,focusRows));
}
install(false);
console.log("PAIR_LOCALDEF_WHNF_TAIL_THRESHOLDS "+JSON.stringify(thresholds));

console.log("PAIR_LOCALDEF_WHNF_TAIL "+JSON.stringify(thresholds));
process.exit(0);
