...
function install(candidate){
  ...
  proto.equal=function(a,b,ctx=[]){
    ...
    const depth=this._mixedSpineDepth??0;
    ...
    // Only a same reducible definition may start speculation. Once inside that
    // one transaction, any exact same-head spine may be decomposed by congruence.
    if(!sameHead || (depth===0&&d?.kind!=="def")) return retained.call(this,a,b,ctx);
...
for(const candidate of [false,true]){
  ...
  for(const row of rows){
    ...
    console.log("MIXED_SPINE_TRIGGER "+JSON.stringify({mode:candidate?"candidate":"baseline",name:row.name,
