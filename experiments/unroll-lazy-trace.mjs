...
wanted={"unroll-versus-evaluate.ndjson"}
...
function headSummary(k,t){
  ...
  return {tag:"const",kind:k.env.get(h[1])?.kind??"const",name:h[1],arity:s.args.length};
}
...
function install(enabled){
  ...
  proto.equal=function(a,b,ctx=[]){
    ...
    for(let n=0;n<12000;n++){
      if(this.__lazyTrace.length<120) this.__lazyTrace.push({event:"state",n,x:headSummary(this,x),y:headSummary(this,y),steps:this.steps,budget:this.budget});
      ...
      if(ky==="def"&&kx!=="def"){
        const ny=reduceHeadOnce(this,y,true);if(ny!==null&&ny!==y){if(this.__lazyTrace.length<120)this.__lazyTrace.push({event:"step-right",n,to:headSummary(this,ny)});y=ny;continue;}
        const nx=reduceHeadOnce(this,x,true);if(nx!==null&&nx!==x){if(this.__lazyTrace.length<120)this.__lazyTrace.push({event:"step-left",n,to:headSummary(this,nx)});x=nx;continue;}
      }else{
        const nx=reduceHeadOnce(this,x,true);if(nx!==null&&nx!==x){if(this.__lazyTrace.length<120)this.__lazyTrace.push({event:"step-left",n,to:headSummary(this,nx)});x=nx;continue;}
        const ny=reduceHeadOnce(this,y,true);if(ny!==null&&ny!==y){if(this.__lazyTrace.length<120)this.__lazyTrace.push({event:"step-right",n,to:headSummary(this,ny)});y=ny;continue;}
      }
...
proto.result=function(...args){const r=oldResult.apply(this,args);r.lazy_trace=this.__lazyTrace??null;return r;};
for(const enabled of [false,true]){
  ...
  for(const row of rows){
    ...
    console.log("LAZY_DELTA_ORDER "+JSON.stringify({mode:enabled?"candidate":"baseline",name:row.name,
      ...
      trace:enabled?(r.lazy_trace??null):null}));
  }
}
proto.equal=fallback;proto.result=oldResult;
