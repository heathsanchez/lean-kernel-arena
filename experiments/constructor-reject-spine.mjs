...
function install(enabled){
  ...
  proto.equal=function(a,b,ctx=[]){
    ...
    const depth=this._ctorRejectDepth??0,sa=rawSpine(a),sb=rawSpine(b);
    ...
    if(!sameHead || (depth===0&&d?.kind!=="inductive")) return retained.call(this,a,b,ctx);
    ...
    try{
      ...
      this._ctorRejectDepth=depth;
      if(outer)this.budget=snap.budget;
      return;
    }catch(e){
      this._ctorRejectDepth=depth;
      if(!(e instanceof Stop||e instanceof RangeError)){if(outer)this.budget=snap.budget;throw e;}
      if(e?.__rigidCtorReject===true){
        if(outer)this.budget=snap.budget;
        throw e;
      }
      if(depth>0&&d?.kind==="ctor"&&e instanceof Stop&&e.status===K.REJECT){
        e.__rigidCtorReject=true;
        throw e;
      }
      if(outer){
...
const baseline=evaluate("baseline",false),candidate=evaluate("constructor-reject",true);
proto.equal=retained;
...
for(let i=0;i<rows.length;i++){
 ...
 if(residual.has(r.name)){
  if(r.status!=="UNKNOWN"){if(r.status!==r.expected)throw new Error("WRONG_RESOLUTION "+JSON.stringify(r));resolved++;resolvedCases.push({name:r.name,status:r.status,reason:r.reason,steps:r.steps,constructed:r.constructed});}
...
const summary={arena_sha256:sha,budget,speculation_cap:CAP,
 ...
 lawful:candidate.wrong===0&&protectedChanged===0,promotable:candidate.wrong===0&&protectedChanged===0&&resolved>0,
 claim_boundary:"Negative execution consequence only: inside one rigid-inductive congruence transaction, a REJECT proven by retained conversion of fields under the exact same constructor head is propagated by constructor injectivity. Other failures roll back to retained conversion."};
mkdirSync(new URL("../genesis/evidence/",import.meta.url),{recursive:true});
writeFileSync(new URL("../genesis/evidence/constructor-reject-spine.json",import.meta.url),JSON.stringify({summary,baseline,candidate},null,2));
console.log("CONSTRUCTOR_REJECT_SPINE "+JSON.stringify(summary));
