// Prospective execution separator: a fully explicit continuation machine for
// nested LocalDef inference. It preserves the exact LocalDef variable/let
// semantics but eliminates JavaScript recursion across app/lam nesting.
...
function install(enabled){
  ...
  proto.infer=function(e,ctx=[]){
    ...
    // Once inside super inference, own nested app/lam/let control flow here.
...
const candidate=evaluate("local-continuation",true);
proto.infer=retainedInfer;
...
const summary={arena_sha256:sha,budget,
  ...
  claim_boundary:"Execution-only control-flow repair. Nested LocalDef app/lambda/let inference uses an explicit continuation machine while preserving the existing variable lookup, local-definition context, typing, conversion, substitution and capability rules."
};
mkdirSync(new URL("../genesis/evidence/",import.meta.url),{recursive:true});
writeFileSync(new URL("../genesis/evidence/localdef-continuation.json",import.meta.url),JSON.stringify({summary,baseline,candidate},null,2));
console.log("LOCALDEF_CONTINUATION "+JSON.stringify(summary));
