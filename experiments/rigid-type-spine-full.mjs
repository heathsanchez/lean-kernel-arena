...
function install(candidate){
  ...
  proto.equal=function(a,b,ctx=[]){
    ...
    const depth=this._rigidTypeSpineDepth??0,sa=rawSpine(a),sb=rawSpine(b);
    ...
    if(outer){snap={steps:this.steps,frontier:this.conversionFrontier,budget:this.budget};this.budget=Math.min(this.budget,this.steps+CAP);}
    ...
    try{
      ...
      this._rigidTypeSpineDepth=depth;if(outer)this.budget=snap.budget;return;
    }catch(e){
...
function evaluate(mode,candidate){
  install(candidate);
...
const baseline=evaluate("baseline",false),candidate=evaluate("rigid-inductive-spine",true);
proto.equal=retained;
...
for(let i=0;i<rows.length;i++){
  ...
  if(residual.has(r.name)){
    if(r.status!=="UNKNOWN"){if(r.status!==r.expected)throw new Error("WRONG_RESOLUTION "+JSON.stringify(r));resolved++;resolvedCases.push({name:r.name,status:r.status,steps:r.steps,constructed:r.constructed});}
...
const summary={arena_sha256:sha,budget,speculation_cap:CAP,
  ...
  claim_boundary:"A same exact rigid inductive head may start one cheapest-first congruence transaction. Inside that 250k speculative budget, exact same-head application spines may recurse by congruence. Failure rolls back steps/frontier and returns to retained conversion; local-definition fallback is untouched."};
mkdirSync(new URL("../genesis/evidence/",import.meta.url),{recursive:true});
writeFileSync(new URL("../genesis/evidence/rigid-type-spine-full.json",import.meta.url),JSON.stringify({summary,baseline,candidate},null,2));
console.log("RIGID_TYPE_SPINE_FULL "+JSON.stringify(summary));
