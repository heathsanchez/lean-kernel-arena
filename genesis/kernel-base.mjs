...
class Kernel {
  ...
  addSingleInductive(d) {
    ...
    if(!this.same(rec.type,derived.recType)) this.equal(rec.type,derived.recType,[]);
    for(let i=0;i<rec.rules.length;i++) {
      ...
      if(!this.same(rr.rhs,derived.ruleBodies[i])) this.equal(rr.rhs,derived.ruleBodies[i],[]);
    }
