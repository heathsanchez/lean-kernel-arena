...
p.run=function(...xs){this.__proof1=[];return run0.apply(this,xs);};
p.equal=function(a,b,ctx=[]){
 try{return eq0.call(this,a,b,ctx);}catch(e){
  if(e?.status==="REJECT"&&prettyName(this.currentDeclaration).endsWith("Char.ofNat._proof_1")){
   this.__proof1.push({reason:e.message,step:this.steps,ctxDepth:ctx.length,aHead:head(a),bHead:head(b),a:sketch(a,14),b:sketch(b,14)});
   this.__proof1=this.__proof1.slice(-8);
  }
  throw e;
 }
};
const result0=p.result;p.result=function(...xs){const r=result0.apply(this,xs);r.__proof1=this.__proof1??[];return r;};
for(const name of ["init-prelude","perf/grind-ring-5"]){
 ...
 console.log("CHAR_PROOF1_REJECT_PROFILE "+JSON.stringify({name,status:r.status,reason:r.reason,steps:r.steps??null,frontier:r.frontier_declaration??null,rows:r.__proof1}));
}
