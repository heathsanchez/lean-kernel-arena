...
for(const [name,budget] of cases){
  ...
  const sum=k=>seen.reduce((n,x)=>n+(x.__binderStats?.[k]??0),0);
  const sum2=k=>seen.reduce((n,x)=>n+(x[k]??0),0);
  console.log("EXACT_BINDER_TRANSPORT "+JSON.stringify({
    ...
    shiftHits:sum("shiftHits"),shiftMisses:sum("shiftMisses"),
    substHits:sum("substHits"),substMisses:sum("substMisses"),
    rangeHits:sum("rangeHits"),rangeMisses:sum("rangeMisses"),rangeSkips:sum("rangeSkips"),
    nodeHits:sum2("__compiledNodeHits"),spineHits:sum2("__compiledSpineHits"),
    nativeNatHits:sum2("__nativeNatHits"),ctorIdxHits:sum2("__ctorIdxHits"),
