...
for(const [name,want] of TARGETS){
  ...
  const agg={maskHits:0,maskStores:0,skipped:0,visited:0,queries:0};
  for(const k of seen)for(const q of Object.keys(agg))agg[q]+=k.__fvStats?.[q]??0;
  ...
  const row={name,want,status:r.status,reason:r.reason,steps:r.steps??null,constructed:r.constructed??null,
    elapsed_ms:Date.now()-t0,fv:agg,headBetaSpines:sum("__headBetaSpines"),
...
const out={experiment:"shared-fv-mask-substitution",sharedClosed,protectedClean,promotable:sharedClosed&&protectedClean,rows,
 claim_boundary:"Exact de-Bruijn substitution with a cached free-variable bit mask on immutable term nodes. A subtree is returned by pointer identity only when the target binder is absent and no higher free variable requires decrementing; otherwise retained constructor-by-constructor substitution is reproduced. First-time mask analysis is budget-accounted one tick per unseen node. Composed with corrected beta-spine sharing and exact recursor-prefix reuse."};
mkdirSync("genesis/evidence",{recursive:true});
writeFileSync("genesis/evidence/shared-fv-mask-substitution.json",JSON.stringify(out,null,2)+"\n");
console.log("SHARED_FV_MASK_SUBSTITUTION "+JSON.stringify(out));
