...
const profileMethods=rows.map(r=>r.attempts.at(-1)?.top_method_ticks_for_profiled_declaration??[]);
const report={
  schema:"deep-list-succ-le-succ-method-profile-v1",
  profiled_declaration:PROFILE_DECL,
  candidate:"method-level tick decomposition inside the common Nat.succ_le_succ hotspot after exact symbolic Nat semantic repair",
  claim_boundary:"Diagnostic-only method attribution inside one already observed common declaration hotspot. No semantic rule, cache admission, ACCEPT/REJECT authority, or optimization is added.",
  rows,
  terminal_frontiers:terminalFrontiers,
  method_profiles:profileMethods,
  gates:{
    ...
    profile_nonempty:profileMethods.every(r=>r.length>0),
    same_dominant_method:profileMethods.length===2&&profileMethods[0][0]?.method===profileMethods[1][0]?.method,
    common_terminal_frontier:terminalFrontiers.length===2&&terminalFrontiers[0]!==null&&terminalFrontiers[0]===terminalFrontiers[1],
  },
};
mkdirSync(dirname("genesis/evidence/deep-list-succ-le-succ-method-profile-v1.json"),{recursive:true});
writeFileSync("genesis/evidence/deep-list-succ-le-succ-method-profile-v1.json",JSON.stringify(report,null,2)+"\n");
console.log("DEEP_LIST_SUCC_LE_SUCC_METHOD_PROFILE_RESULT="+JSON.stringify(report));
if(!Object.values(report.gates).every(Boolean))process.exit(1);
console.log("PASS_DEEP_LIST_SUCC_LE_SUCC_METHOD_PROFILE_V1");
