...
for(const [name,budget] of [
 ...
 const stats={};
 for(const k of seen)for(const [q,v] of Object.entries(k.__semStats??{}))stats[q]=(stats[q]??0)+v;
 console.log("COMPILED_SEMANTIC_SEPARATOR "+JSON.stringify({
  name,budget,status:r.status,reason:r.reason,steps:r.steps??null,constructed:r.constructed??null,
  parse_records:r.parse_records??null,frontier:r.frontier_declaration??null,stats,
