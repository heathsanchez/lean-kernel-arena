...
import "./lazy-pi-conversion-layer.mjs";
...
for(const [name,budget] of [
 ...
 const q={attempts:0,hits:0,whnfSame:0,fallbacks:0,failedSpeculations:0};
 for(const k of seen)for(const [n,v] of Object.entries(k.__lazyPiStats??{}))q[n]+=v;
 console.log("LAZY_PI_SEPARATOR "+JSON.stringify({
  name,budget,status:r.status,reason:r.reason,steps:r.steps??null,constructed:r.constructed??null,
  parse_records:r.parse_records??null,frontier:r.frontier_declaration??null,lazyPi:q,
