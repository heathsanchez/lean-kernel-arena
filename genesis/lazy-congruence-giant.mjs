...
for(const [name,budget] of [
 ...
 ["perf/grind-ring-5",4_000_000]
  ...
  const s=k=>seen.reduce((n,x)=>n+(x.__lazyConvStats?.[k]??0),0);
  console.log("LAZY_CONGRUENCE_GIANT "+JSON.stringify({
    ...
    attempts:s("attempts"),headSame:s("headSame"),whnfSame:s("whnfSame"),app:s("app"),
    pi:s("pi"),lam:s("lam"),proj:s("proj"),fallbacks:s("fallbacks"),appFallbacks:s("appFallbacks"),
