...
const methods=["validate","shift","substitute","lowerBound","whnf","whnfMajor","unfoldTheoremHead","same","normal",
"proofType","equal","instantiateDeclaration","sortOf","infer","inferProjection","getApp","make","appN",
"isUnitLikeName","isUnitLikeType","structureEtaMatches"];
for(const n of methods){
 if(typeof p[n]!=="function")continue;
 const f=p[n];
 p[n]=function(...args){
  this.__fullProfile??={stack:[],calls:{},ticks:{}};
  const q=this.__fullProfile;q.calls[n]=(q.calls[n]??0)+1;q.stack.push(n);
...
p.tick=function(...args){
 this.__fullProfile??={stack:[],calls:{},ticks:{}};
 const q=this.__fullProfile,n=q.stack.at(-1)??"<other>";
 q.ticks[n]=(q.ticks[n]??0)+1;
 return tick0.apply(this,args);
};
const result0=p.result;
p.result=function(...args){const r=result0.apply(this,args);r.__fullProfile=this.__fullProfile??null;return r;};

for(const [name,budget] of [["init-prelude",1_000_000],["perf/grind-ring-5",1_000_000]]){
 const input=readFileSync(new URL("../_build/tests/"+name+".ndjson",import.meta.url),"utf8");
 const t0=Date.now(),r=K.checkExport(input,CAPS,budget),q=r.__fullProfile;
 const top=q?Object.entries(q.ticks).sort((a,b)=>b[1]-a[1]).slice(0,24)
  .map(([op,ticks])=>({op,ticks,calls:q.calls[op]??0,share:ticks/Math.max(1,r.steps??1)})):[];
 console.log("CURRENT_GIANT_PROFILE "+JSON.stringify({name,status:r.status,reason:r.reason,steps:r.steps??null,
