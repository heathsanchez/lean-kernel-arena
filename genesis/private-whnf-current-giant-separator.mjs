...
await import("./private-whnf-environment-layer.mjs");
...
for(const [name,expected] of targets){
  ...
  rows.push({name,expected,status:r.status,reason:r.reason,steps:r.steps??null,constructed:r.constructed??null,
    ...
    kernels:seen.map(k=>({localDefs:k.localDefs===true,steps:k.steps??null,constructed:k.allocations??null,
      privateEnv:k.__privateStats??null}))});
}
const wrong=rows.filter(r=>r.status!=="UNKNOWN"&&r.status!==r.expected).length;
const controlClean=rows[2]?.status==="ACCEPT";
const out={experiment:"private-whnf-current-giant",budget,wrong,controlClean,rows,
 claim_boundary:"Requalification only: existing private closure evaluator intercepts its exact five-argument depth-zero WHNF substitution target on the current repaired production stack. No promotion implied."};
const path=new URL(`./evidence/private-whnf-current-${budget}.json`,import.meta.url);
mkdirSync(dirname(path.pathname),{recursive:true});writeFileSync(path,JSON.stringify(out,null,2)+"\n");
console.log("PRIVATE_WHNF_CURRENT "+JSON.stringify(out));
