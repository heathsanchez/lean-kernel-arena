...
import "./whnf-nat-zero-consequence-layer.mjs";

const mode=process.env.CACHE_MODE??"sort";
if(mode==="sort") await import("./context-sort-cache-layer.mjs");
else if(mode==="both") {await import("./context-infer-cache-layer.mjs");await import("./context-sort-cache-layer.mjs");}
else throw new Error("unknown CACHE_MODE "+mode);
...
p.result=function(...xs){const r=result0.apply(this,xs);r.__inferCacheStats=this.__inferCacheStats??null;r.__sortCacheStats=this.__sortCacheStats??null;r.__whnfNatZeroHits=this.__whnfNatZeroHits??0;return r;};
for(const name of ["init-prelude","perf/grind-ring-5","perf/shared-subterm"]){
  ...
  console.log("CONTEXT_CACHE_TOURNAMENT "+JSON.stringify({mode,name,status:r.status,reason:r.reason,steps:r.steps??null,
    frontier:r.frontier_declaration??null,zeroHits:r.__whnfNatZeroHits,infer:r.__inferCacheStats,sort:r.__sortCacheStats}));
}
