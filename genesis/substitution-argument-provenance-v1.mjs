...
const provenanceProfiles=rows.map(r=>r.argument_provenance);
const sameDominant=
  provenanceProfiles.length===2 &&
  provenanceProfiles[0][0]?.origin!==undefined &&
  provenanceProfiles[0][0]?.origin===provenanceProfiles[1][0]?.origin;
const singleDominant=
  sameDominant &&
  rows.every((r,i)=>
    (provenanceProfiles[i][0]?.count??0)/r.argument_family_sampled>0.50
  );
const stableSmallBasis=
  !singleDominant &&
  sameDominant &&
  rows.every(r=>r.argument_provenance_top3_coverage>=0.80);
const classification=
  singleDominant ? "single-generator-dominant" :
  stableSmallBasis ? "stable-small-generator-basis" :
  "distributed-generation";

const report={
  schema:"substitution-argument-provenance-v1",
  precommit:{
    realitygraph_commit:"71fa248c183950e0757bc0bef76fd0129eecbd8e",
    source_episode_run:35433654472,
    source_episode_artifact:10581074870,
    source_episode_digest:"sha256:27f5a6d0ac9587351f67ea28923486db0e3b2eb0b8e19e7e4c28c7fc440c4a73",
  },
  semantic_context:"qualified exact symbolic Nat repair plus exact substitution argument-family instrumentation",
  candidate:"tag successfully returned Kernel.make objects with the active parent kernel method and read the tag only for sampled newArgument cache-key events",
  claim_boundary:"Diagnostic object-origin tagging only. Provenance labels are not semantic equality and do not change construction, structural sharing, substitution or cache admission. source-or-untracked includes parsed/input objects and objects created outside the instrumented make path.",
  rows,
  provenance_profiles:provenanceProfiles,
  classification,
  gates:{
    no_wrong_reject:rows.every(r=>r.status!=="REJECT"),
    both_unknown_at_4m:rows.every(r=>r.status==="UNKNOWN"&&r.steps===4_000_001),
    bounded_provenance_sample:rows.every(r=>r.argument_family_sampled>50_000&&r.argument_family_sampled<=200_000),
    provenance_counts_exact:rows.every(r=>r.argument_provenance.reduce((n,x)=>n+x.count,0)===r.argument_family_sampled),
    classification_total:["single-generator-dominant","stable-small-generator-basis","distributed-generation"].includes(classification),
  },
};
mkdirSync(dirname("genesis/evidence/substitution-argument-provenance-v1.json"),{recursive:true});
writeFileSync("genesis/evidence/substitution-argument-provenance-v1.json",JSON.stringify(report,null,2)+"\n");
console.log("SUBSTITUTION_ARGUMENT_PROVENANCE_RESULT="+JSON.stringify(report));
if(!Object.values(report.gates).every(Boolean))process.exit(1);
console.log("PASS_SUBSTITUTION_ARGUMENT_PROVENANCE_V1");
