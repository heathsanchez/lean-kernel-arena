...
for(const name of ["init-prelude","perf/grind-ring-5","perf/shared-subterm"]){
 ...
 console.log("NATIVE_NAT_OFFSET_SEPARATOR "+JSON.stringify({name,status:r.status,reason:r.reason,
   steps:r.steps??null,constructed:r.constructed??null,parse_records:r.parse_records??null,
   frontier:r.frontier_declaration??null,nativeNatHits:seen.reduce((n,x)=>n+(x.__nativeNatHits??0),0),
   offsetHits:seen.reduce((n,x)=>n+(x.__natOffsetHits??0),0),elapsed_ms:Date.now()-t0}));
}
