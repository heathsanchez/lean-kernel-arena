...
async function evaluate(candidate){
 if(candidate)await import("./hashcons-hit-reuse-layer.mjs");
 const rows=[];
 for(const [name,want,focus] of CASES){
  ...
  const make={hits:0,misses:0};
  if(candidate)for(const k of seen)for(const q of Object.keys(make))make[q]+=k.__makeReuseStats?.[q]??0;
  rows.push({name,want,focus,status:r.status,reason:r.reason,steps:r.steps??null,constructed:r.constructed??null,
    elapsed_ms:Date.now()-t0,make,headBetaSpines:seen.reduce((n,k)=>n+(k.__headBetaSpines??0),0),
...
const comparisons=CASES.map(([name,want,focus],i)=>{
 ...
 after:{status:c.status,reason:c.reason,steps:c.steps,constructed:c.constructed,elapsed_ms:c.elapsed_ms},make:c.make,
 headBetaSpines:c.headBetaSpines,prefixHits:c.prefixHits,statusChanged:b.status!==c.status,
...
const out={experiment:"hashcons-hit-only",budget:1_000_000,sharedClosed:shared.after.status==="ACCEPT",
 protectedChanged:changed.length,protectedWrong:wrong.length,lawful:changed.length===0&&wrong.length===0,
 promotable:shared.after.status==="ACCEPT"&&changed.length===0&&wrong.length===0,comparisons,
 claim_boundary:"Exact structural hash-cons hits reuse the already-created immutable node without another budget tick; only misses tick and allocate. No term semantics, equality, typing, reduction, or rejection rule changes."};
mkdirSync(new URL("./evidence/",import.meta.url),{recursive:true});
writeFileSync(new URL("./evidence/hashcons-hit-only.json",import.meta.url),JSON.stringify(out,null,2)+"\n");
console.log("HASHCONS_HIT_ONLY "+JSON.stringify(out));
if(!out.lawful)process.exit(1);
