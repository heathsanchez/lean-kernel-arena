...
p.result=function(...xs){const r=result0.apply(this,xs);r.__earlyPIStats=this.__earlyPIStats??null;r.__semStats=this.__semStats??null;r.__whnfNatZeroHits=this.__whnfNatZeroHits??0;return r;};
for(const name of ["init-prelude","perf/grind-ring-5","perf/shared-subterm"]){
  ...
  console.log("EARLY_PI_PROFILE "+JSON.stringify({name,status:r.status,reason:r.reason,steps:r.steps??null,frontier:r.frontier_declaration??null,
    zeroHits:r.__whnfNatZeroHits,earlyPI:r.__earlyPIStats,semantic:r.__semStats}));
}
