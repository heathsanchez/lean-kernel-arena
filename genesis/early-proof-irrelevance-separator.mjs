...
for(const [name,budget] of [
 ...
 const q={attempts:0,hits:0,nonProof:0,probeFallbacks:0,typeFallbacks:0};
 for(const k of seen)for(const [n,v] of Object.entries(k.__earlyPIStats??{}))q[n]+=v;
 console.log("EARLY_PROOF_IRRELEVANCE_SEPARATOR "+JSON.stringify({
  name,budget,status:r.status,reason:r.reason,steps:r.steps??null,constructed:r.constructed??null,
  parse_records:r.parse_records??null,frontier:r.frontier_declaration??null,earlyPI:q,
  proofHits:seen.reduce((n,k)=>n+(k.__semStats?.proofHits??0),0),
  proofStores:seen.reduce((n,k)=>n+(k.__semStats?.proofStores??0),0),
