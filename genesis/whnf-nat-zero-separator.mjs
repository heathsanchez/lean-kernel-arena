...
for(const name of ["init-prelude","perf/grind-ring-5","perf/shared-subterm"]){
  ...
  console.log("WHNF_NAT_ZERO_SEPARATOR "+JSON.stringify({name,status:r.status,reason:r.reason,
    steps:r.steps??null,frontier:r.frontier_declaration??null,hits:r.__whnfNatZeroHits}));
}
