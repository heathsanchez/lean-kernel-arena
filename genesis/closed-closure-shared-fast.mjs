...
for(const k of seen)for(const q of Object.keys(st))st[q]+=k.__closedClosureStats?.[q]??0;
console.log("CLOSED_CLOSURE_SHARED_FAST "+JSON.stringify({status:r.status,reason:r.reason,steps:r.steps??null,constructed:r.constructed??null,elapsed_ms:Date.now()-t0,stats:st}));
