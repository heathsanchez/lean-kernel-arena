...
import "./whnf-app-congruence-before-normal-layer.mjs";
...
for(const [name,budget] of [
 ...
 const q={attempts:0,sameHits:0,appAttempts:0,appHits:0,appFallbacks:0,fallbacks:0};
 for(const k of seen)for(const [n,v] of Object.entries(k.__whnfAppStats??{}))q[n]+=v;
 console.log("NAT_CLASS_WHNF_APP_SEPARATOR "+JSON.stringify({
  name,budget,status:r.status,reason:r.reason,steps:r.steps??null,constructed:r.constructed??null,
  parse_records:r.parse_records??null,frontier:r.frontier_declaration??null,whnfApp:q,
