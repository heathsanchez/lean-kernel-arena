...
const report={
  ...
  gates:{
    ...
    dominant_method_majority:profileMethods.every(r=>{
      const total=r.reduce((n,x)=>n+x.ticks,0);
      return total>0&&(r[0]?.ticks??0)/total>=0.50;
    }),
    common_terminal_frontier:terminalFrontiers.length===2&&terminalFrontiers[0]!==null&&terminalFrontiers[0]===terminalFrontiers[1],
  },
};
mkdirSync(dirname("genesis/evidence/isvalidchar-method-profile-v1.json"),{recursive:true});
writeFileSync("genesis/evidence/isvalidchar-method-profile-v1.json",JSON.stringify(report,null,2)+"\n");
console.log("ISVALIDCHAR_METHOD_PROFILE_RESULT="+JSON.stringify(report));
if(!Object.values(report.gates).every(Boolean))process.exit(1);
console.log("PASS_ISVALIDCHAR_METHOD_PROFILE_V1");
