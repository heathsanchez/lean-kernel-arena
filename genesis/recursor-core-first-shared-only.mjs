...
for(const budget of [1_000_000,2_000_000]){
  ...
  const slot={queries:0,hits:0,dynamicNodes:0,prefixReused:0};
  for(const k of seen){
    for(const q of Object.keys(core))core[q]+=k.__recCoreStats?.[q]??0;
    for(const q of Object.keys(slot))slot[q]+=k.__slotStats?.[q]??0;
  }
  console.log("SHARED_ONLY "+JSON.stringify({budget,status:r.status,reason:r.reason,steps:r.steps??null,
    constructed:r.constructed??null,elapsed_ms:Date.now()-t0,core,slot}));
