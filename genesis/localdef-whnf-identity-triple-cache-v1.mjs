...
for(const name of TARGETS){
  ...
  for(const arm of ["cold","string-key","fast-identity","restart-fast","ablation"]){
    Kernel.prototype.whnf=
      arm==="string-key" ? cachedLocalWhnf :
      (arm==="fast-identity"||arm==="restart-fast") ? fastIdentityLocalWhnf :
      semanticRepairWhnf;
    ...
    const stringHits=seen.reduce((n,k)=>n+(k.__localWhnfCacheHits??0),0);
    const stringStores=seen.reduce((n,k)=>n+(k.__localWhnfCacheStores??0),0);
    ...
    arms[arm]={
      ...
      elapsed_ms:elapsed,
      string_cache_hits:stringHits,
      string_cache_stores:stringStores,
      fast_cache_hits:fastHits,
      fast_cache_stores:fastStores,
    };
    console.log("LOCALDEF_WHNF_IDENTITY_TRIPLE_ARM "+JSON.stringify({
      name,arm,...arms[arm]
    }));
  }
  rows.push({name,arms});
...
const report={
  schema:"localdef-whnf-identity-triple-cache-v1",
  source_observation:{
    run_id:35420366767,
    artifact_id:10576707760,
    artifact_digest:"sha256:6f0e59af5595d59b712cee43a807c95aa292a164a9d374543001e4c0294f5bb3",
    string_key_hits:[1438711,1438713],
    observation:"string-keyed exact localdef cache produced many hits but no frontier movement and added wall-time overhead",
  },
  candidate:"replace serialized declaration/params/context key construction with exact expression identity × active-context-array identity × universe-params-set identity",
  claim_boundary:"Candidate-only exact-identity cache-key compression. The fast key is stricter than the prior string key: no structural context equivalence is inferred. Only successful WHNF consequences are cached; failures, UNKNOWN, REJECT and cross-context results are not.",
  rows,
  comparison:{
    string_elapsed_ms:sum("string-key","elapsed_ms"),
    fast_elapsed_ms:sum("fast-identity","elapsed_ms"),
    cold_elapsed_ms:sum("cold","elapsed_ms"),
    string_hits:sum("string-key","string_cache_hits"),
    fast_hits:sum("fast-identity","fast_cache_hits"),
    restart_fast_hits:sum("restart-fast","fast_cache_hits"),
  },
  gates:{
    all_semantic_statuses_match:rows.every(r=>new Set(Object.values(r.arms).map(a=>a.status)).size===1),
    no_wrong_reject:rows.every(r=>Object.values(r.arms).every(a=>a.status!=="REJECT")),
    fast_hits_large:rows.every(r=>r.arms["fast-identity"].fast_cache_hits>100_000),
    restart_fast_reproduces:rows.every(r=>
      r.arms["restart-fast"].status===r.arms["fast-identity"].status &&
      r.arms["restart-fast"].reason===r.arms["fast-identity"].reason &&
      r.arms["restart-fast"].fast_cache_hits>100_000
    ),
    ablation_has_zero_fast_hits:rows.every(r=>r.arms.ablation.fast_cache_hits===0),
    fast_key_beats_string_key_wall_time:
      sum("fast-identity","elapsed_ms")<sum("string-key","elapsed_ms"),
  },
};
mkdirSync(dirname("genesis/evidence/localdef-whnf-identity-triple-cache-v1.json"),{recursive:true});
writeFileSync("genesis/evidence/localdef-whnf-identity-triple-cache-v1.json",JSON.stringify(report,null,2)+"\n");
console.log("LOCALDEF_WHNF_IDENTITY_TRIPLE_CACHE_RESULT="+JSON.stringify(report));
if(!Object.values(report.gates).every(Boolean))process.exit(1);
console.log("PASS_LOCALDEF_WHNF_IDENTITY_TRIPLE_CACHE_V1");
