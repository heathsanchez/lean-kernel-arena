...
for(const [name,budget] of [
 ...
 const q={attempts:0,sameHits:0,appAttempts:0,appHits:0,appFallbacks:0,projHits:0,piHits:0,lamHits:0,fallbacks:0};
 for(const k of seen)for(const [n,v] of Object.entries(k.__whnfCongStats??{}))q[n]+=v;
 console.log("WHNF_CONGRUENCE_BEFORE_NORMAL_SEPARATOR "+JSON.stringify({
  name,budget,status:r.status,reason:r.reason,steps:r.steps??null,constructed:r.constructed??null,
  parse_records:r.parse_records??null,frontier:r.frontier_declaration??null,whnfCong:q,
