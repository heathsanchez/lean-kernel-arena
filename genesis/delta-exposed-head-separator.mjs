...
import "./delta-exposed-head-congruence-layer.mjs";
...
for(const [name,budget] of [
 ...
 const q={attempts:0,headChanges:0,sameHits:0,appAttempts:0,appHits:0,appFallbacks:0,fallbacks:0};
 for(const k of seen)for(const [n,v] of Object.entries(k.__exposedHeadStats??{}))q[n]+=v;
 console.log("DELTA_EXPOSED_HEAD_SEPARATOR "+JSON.stringify({
  name,budget,status:r.status,reason:r.reason,steps:r.steps??null,constructed:r.constructed??null,
  parse_records:r.parse_records??null,frontier:r.frontier_declaration??null,exposed:q,
