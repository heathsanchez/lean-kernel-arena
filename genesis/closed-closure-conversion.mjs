...
async function evalAll(candidate){
 if(candidate)await import("./closed-closure-conversion-layer.mjs");
 const rows=[];
 for(const [name,want,focus] of CASES){
  ...
  const st={attempts:0,successes:0,aborts:0,ops:0,ctorPairs:0,whnfHits:0,recs:0,beta:0};
  if(candidate)for(const k of seen)for(const q of Object.keys(st))st[q]+=k.__closedClosureStats?.[q]??0;
...
const out={experiment:"closed-closure-conversion",sharedClosed:shared.after.status==="ACCEPT",protectedChanged:changed.length,protectedWrong:wrong.length,lawful:changed.length===0&&wrong.length===0,promotable:shared.after.status==="ACCEPT"&&changed.length===0&&wrong.length===0,comparisons:cmp};
mkdirSync(new URL("./evidence/",import.meta.url),{recursive:true});writeFileSync(new URL("./evidence/closed-closure-conversion.json",import.meta.url),JSON.stringify(out,null,2)+"\n");
console.log("CLOSED_CLOSURE_CONVERSION "+JSON.stringify(out));if(!out.promotable)process.exit(1);
