...
import "./context-infer-cache-layer.mjs";
...
p.result=function(...xs){const r=result0.apply(this,xs);r.__inferCacheStats=this.__inferCacheStats??null;r.__whnfNatZeroHits=this.__whnfNatZeroHits??0;return r;};
for(const name of ["init-prelude","perf/grind-ring-5","perf/shared-subterm"]){
  ...
  console.log("INFER_CACHE_SEPARATOR "+JSON.stringify({name,status:r.status,reason:r.reason,steps:r.steps??null,
    frontier:r.frontier_declaration??null,zeroHits:r.__whnfNatZeroHits,cache:r.__inferCacheStats}));
}
