...
import "./normalized-nat-zero-consequence-layer.mjs";
...
p.result=function(...xs){const r=result0.apply(this,xs);r.__normalizedNatZeroHits=this.__normalizedNatZeroHits??0;return r;};
...
for(const name of ["init-prelude","perf/grind-ring-5","perf/shared-subterm"]){
  ...
  console.log("NORMALIZED_NAT_ZERO_SEPARATOR "+JSON.stringify({name,status:r.status,reason:r.reason,
    steps:r.steps??null,frontier:r.frontier_declaration??null,hits:r.__normalizedNatZeroHits}));
}
