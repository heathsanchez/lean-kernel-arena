...
import "./root-whnf-before-normal-layer.mjs";
...
for(const [name,budget] of [
 ...
 const q={attempts:0,hits:0,changedLeft:0,changedRight:0,fallbacks:0};
 for(const k of seen)for(const [n,v] of Object.entries(k.__rootWhnfEqStats??{}))q[n]+=v;
 console.log("ROOT_WHNF_BEFORE_NORMAL_SEPARATOR "+JSON.stringify({
  name,budget,status:r.status,reason:r.reason,steps:r.steps??null,constructed:r.constructed??null,
  parse_records:r.parse_records??null,frontier:r.frontier_declaration??null,rootWhnf:q,
