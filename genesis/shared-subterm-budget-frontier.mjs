...
function shape(e){
  ...
  return {tag:e[0],head:Array.isArray(h)?(h[0]==="const"?["const",h[1]]:h.slice(0,2)):typeof h,args:n};
}
...
p.equal=function(a,b,ctx=[]){
  if(this.steps>=850000 && (this.__frontierSamples?.length??0)<40)
    this.__frontierSamples.push({op:"equal",steps:this.steps,decl:this.currentDeclaration,ctx:ctx.length,a:shape(a),b:shape(b)});
  this.__lastEqual={steps:this.steps,decl:this.currentDeclaration,ctx:ctx.length,a:shape(a),b:shape(b)};
  return oldEqual.call(this,a,b,ctx);
};
p.normal=function(e){
  if(this.steps>=850000 && (this.__frontierSamples?.length??0)<40)
    this.__frontierSamples.push({op:"normal",steps:this.steps,decl:this.currentDeclaration,e:shape(e)});
  this.__lastNormal={steps:this.steps,decl:this.currentDeclaration,e:shape(e)};
